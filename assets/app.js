 ...
 if (selectedFiles.length > 0) {
  ...
  for (let i = 0; i < uploadedImages.length; i++) {
    ...
    if (!imageLinkResponse.ok || !imageLinkData.success) {
      throw new Error(
        imageLinkData.error ||
        'Una imagen se subió, pero no fue posible asociarla al producto.'
      );
    }
  }
}
