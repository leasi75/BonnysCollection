...
async function openProduct(id){let p=P.find(x=>x.id===id);if(!p)return;try{
  ...
  p.inventory=[];
}document.querySelector('#modalBody').innerHTML=`<div class="detail"><div class="gallery ${p.category==='Accesorios'?'accessoryGallery':''}">${p.images.map(i=>`<img src="${i}" alt="${esc(p.name)}">`).join('')}</div><div class="detailText"><small>${p.category}</small><h2>${esc(p.name)}</h2><p>${esc(p.description||'')}</p><p><b>Color:</b> ${esc(p.color||p.colors||'Por confirmar')}</p>${p.sizes?`<p><b>Tallas:</b> ${esc(p.sizes)}</p>`:''}${p.price?`<p class="detailPrice"><b>Precio:</b> $${esc(p.price)}</p>`:''}<p class="stock ${Number(p.stock)<=0?'out':''}"><b>${stockText(p)}</b></p><label>Talla</label><select>${Array.isArray(p.inventory) && p.inventory.length ? p.inventory.filter(item=>Number(item.stock)>0).map(item=>`<option value="${esc(item.size)}">${esc(item.size)} — ${item.stock} disponible${Number(item.stock)===1?'':'s'}</option>`).join('') : '<option>Disponibilidad por confirmar</option>'}</select><button ${Number(p.stock)<=0?'disabled':''} onclick="addCart('${p.id}');closeModal()">${Number(p.stock)<=0?'Producto agotado':'Agregar al pedido'}</button></div></div>`;document.querySelector('#modal').classList.remove('hidden')}
